...
void __fastcall TForm11::CreateNewRecomMeal(int num) {
	...
	Form11->RecomMeal[n].Weight = new TEdit(Form11); // маса блюда
...
void __fastcall TForm11::UpdateWeights(int num)
{
	...
	if (Form8->BMR - allcal > 400) // если меньше
	{
		...
		Form11->Label11->Caption =
			"В сумі меньше ккал,ніж ваша денна норма (" + IntToStr(allcal)
			+ "ккал)";
		Form11->Label11->Font->Color = clWhite;
	}
	else if (Form8->BMR - allcal < -400) // если больше
	{
		Form11->RecomPanel->Color = RGB(247, 192, 25);
		Form11->Label11->Caption =
			"В сумі більше ккал,ніж ваша денна норма (" + IntToStr(allcal)
			+ "ккал)";
		Form11->Label11->Font->Color = clWhite;
	}
	else {
		Form11->RecomPanel->Color = RGB(25, 247, 83); // если идеально
		Form11->Label11->Caption =
			"В сумі рекомендована кількість ккалорій (" + IntToStr(allcal)
			+ "ккал)";
...
void __fastcall TForm11::Excel1Click(TObject *Sender) {
	ExcelInit("");
	Sh.OlePropertySet("Name","Страви");
	toExcelCell(2, 4, "Розподіл по стравам");
	toExcelCell(4,1,"Назва страви");
	toExcelCell(4, 2, "Маса(г)");
	toExcelCell(4, 3, "Білків(г)");
	toExcelCell(4, 4, "Жирів(г)");
	toExcelCell(4, 5, "Вуглеводів(г)");
	toExcelCell(4, 6, "Ккалорій");

	Variant VarCell=Sh.OlePropertyGet("Range","A2:F3");
	VarCell.OleProcedure("Merge");  //обьеденяем
	VarCell.OlePropertySet("HorizontalAlignment", -4108);//выравнивание по центру
	...
	VarCell=Sh.OlePropertyGet("Range",Sh.OlePropertyGet("Cells",5,1),Sh.OlePropertyGet("Cells",CurRow-1,6)); //Выравнивание текста
	...
	VarCell=Sh.OlePropertyGet("Range",Sh.OlePropertyGet("Cells",4,1),Sh.OlePropertyGet("Cells",CurRow-1,6)); //граница блюд
	...
	VarCell=Sh.OlePropertyGet("Range",Sh.OlePropertyGet("Cells",CurRow,1),Sh.OlePropertyGet("Cells",CurRow,6)); //граница суммирования
	VarCell.OlePropertyGet("Borders",11).OlePropertySet("LineStyle",1);
	...
	toExcelCell(3,1,"Назва продукту");
	toExcelCell(3,2,"Маса(г)");
	...
	for(int i=0;i<RecomMeal.size();i++)
	{
	   ...
	   VarCell=Sh.OlePropertyGet("Range",Sh.OlePropertyGet("Cells",FirstRow,1),Sh.OlePropertyGet("Cells",CurRow-1,2)); //границы
	   VarCell.OlePropertyGet("Borders",11).OlePropertySet("LineStyle",1);
	   VarCell.OlePropertyGet("Borders",11).OlePropertySet("Weight",2);
	   VarCell.OlePropertyGet("Borders",12).OlePropertySet("LineStyle",1);
	   VarCell.OlePropertyGet("Borders",12).OlePropertySet("Weight",2);
...
//////////////////////////////////////////////////////////////////////////////// Необхідні продукти
	...
	Sh.OlePropertySet("Name","Необхідні продукти");
	toExcelCell(2,1,"Необхідні продукти для повного приготування");
	VarCell=Sh.OlePropertyGet("Range","A2:B2");
	VarCell.OleProcedure("Merge");
	VarCell.OlePropertySet("HorizontalAlignment", -4108);//выравнивание по центру
	VarCell.OlePropertySet("VerticalAlignment",-4108);
    ...
    VarCell=Sh.OlePropertyGet("Range",Sh.OlePropertyGet("Cells",3,1),Sh.OlePropertyGet("Cells",CurRow-1,2)); //границы
	VarCell.OlePropertyGet("Borders",11).OlePropertySet("LineStyle",1);
	VarCell.OlePropertyGet("Borders",11).OlePropertySet("Weight",2);
	VarCell.OlePropertyGet("Borders",12).OlePropertySet("LineStyle",1);
	VarCell.OlePropertyGet("Borders",12).OlePropertySet("Weight",2);
