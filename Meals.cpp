...
void AddLeft(int num) {      //создаем новые панели в левой части
	...
	Form3->ProdHave[n].Panel->Tag=num;    //сохран€ем прив€зку к Products(num-номер продукта)
	...
	Form3->ProdHave[n].Protein=new TLabel(Form6);                     //лейблы хран€т белки,жири,калор.
...
void __fastcall TForm3::CreateNewMeal()       //создаем новое невидимое блюдо(если нажмем сохранить,то оно станет видимым)
{
	...
	// нужно об€зательно,иначе по€вл. пропуски
...
void __fastcall TForm3::AddClickF(int num)
{
   ...
   mealNum=PropNum;
	...
	if(Meal[mealNum].IsLeft[num]==true)
	{
		...
		ProdHave[num].Panel->Parent=Form6->ScrollBox2; //прив€зка к правому scrollbox
		for(int j=0;j<ProductsUse+ProductsHave;j++)
		if(Meal[mealNum].IsLeft[j] && ProdHave[j].Panel->Top > ProdHave[num].Panel->Top)  //измен€ем высоту панелей слева
	...
	else
	{
		...
		for(int j=0;j<ProductsUse+ProductsHave;j++)
		if(Meal[mealNum].IsRight[j] && ProdHave[j].Panel->Top > ProdHave[num].Panel->Top)   //измен€ем высоту панелей справа
			ProdHave[j].Panel->Top-=PanelHaveH;
...
void __fastcall TForm3::PropClickNum(int num)
{
  ...
  memset(LeftWas,false,200*sizeof(bool));                 //нужно,чтоб при отмене действи€ удал€лись
...
if(Form3->Components[FocusIndex]->ClassName()=="TEdit")
	{
	  ...
	  s.Delete(start+1,edit->SelLength);    //если выделено,то замен€ем
	  s.Insert(button->Caption,start+1);    //вставл€ем букву
	  edit->SetFocus();
	  edit->Text=s;
